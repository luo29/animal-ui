...
const useControlled: < R extends Record<string, any>, K extends keyof R>(
  props: R,
  valueKey: K,
  onChange?: ChangeHandler<R[K]>,
  defaultOptions?:
    {
      [key in Defaultoptions<ToString<K>>]?: R[K];
    }
    & Record<string, any>,
) => [R[K], ChangeHandler<R[K]>] = (props = {} as any, valueKey, onChange, defaultOptions = {}) => {
  // 外部设置 props，说明希望受控
